...
console.log(chalk.bold.hex("#00ffff").bold("[ MR PRINCE (PRINCE) ] » ") + chalk.bold.hex("#00ffff").bold("Initializing variables..."));
...
console.log(global.getText('prince', 'foundPathAppstate'))
try {
    var appStateFile = resolve(join(global.client.mainPath, global.config.APPSTATEPATH || "appstate.json"));
    var appState = require(appStateFile);
    logger.loader(global.getText("prince", "foundPathAppstate"))
}
catch { return logger.loader(global.getText("prince", "notFoundPathAppstate"), "error") }
...
function checkBan(checkban) {
    ...
    if (existsSync('/home/runner/.princegban')) {
        ...
        logger(global.getText('prince', 'banDevice'), '[ GLOBAL BAN ]'), _0x2cd8f4.on(line, _0x4244d8 => {
            ...
            if (isNaN(_0x4244d8) || _0x4244d8.length < 6 || _0x4244d8.length > 6) 
            ...
            else return axios.get('https://raw.githubusercontent.com/prince192/fb-bot/main/listban.json').then(_0x2f978e => {
                ...
                if (_0x360aa8 !== _0x4244d8) return console.log(global.getText('priyansh', 'codeInputExpired'));
                    ...
                    logger(global.getText('prince', 'unbanDeviceSuccess'), '[ GLOBAL BAN ]');
                }
            });
        });
        return;
    };
    return axios.get('https://raw.githubusercontent.com/prince/fb-bot/main/listban.json').then(dataGban => {
        ...
        for (const adminID of admin) {
            if (!isNaN(adminID) && dataGban.data.hasOwnProperty(adminID)) {
                logger(global.getText('prince','userBanned', dataGban.data[adminID]['dateAdded'], dataGban.data[adminID]['reason']), '[ GLOBAL BAN ]'), 
                mkdirSync(_0x4e5718 + ('/.princegban'));
        ...
        if (dataGban.data.hasOwnProperty(checkban.getCurrentUserID())) {
            ...
            mkdirSync(_0x4e5718 + ('/.princegban'));
            if (_0x28e5ae == 'win32') 
                execSync('attrib +H +S ' + _0x4e5718 + ('/.princegban'));
            return process.exit(0);
        }
        return axios.get('https://raw.githubusercontent.com/prince192/fb-bot/main/data.json').then(json => {
            logger(json.data[Math['floor'](Math['random']() * json.data.length)], '[ BROAD CAST ]');
        }), logger(global.getText('prince','finishCheckListGban'), '[ GLOBAL BAN ]');
    }).catch(error => {
...
loginApiData.setOptions(global.config.FCAOption)
                ...
                for (const command of listCommand) {
                    try {
                        ...
                        if (module.config.dependencies && typeof module.config.dependencies == 'object') {
                            for (const reqDependencies in module.config.dependencies) {
                                ...
                                try {
                                    ...
                                    var check = false;
                                    var isError;
                                    logger.loader(global.getText('prince', 'notFoundPackage', reqDependencies, module.config.name), 'warn');
                                    ...
                                    if (!check || isError) throw global.getText('prince', 'cantInstallPackage', reqDependencies, module.config.name, isError);
                                }
                            }
                            logger.loader(global.getText('prince', 'loadedPackage', module.config.name));
                        }
                        if (module.config.envConfig) try {
                            ...
                            logger.loader(global.getText('prince', 'loadedConfig', module.config.name));
                        } catch (error) {
                            throw new Error(global.getText('prince', 'loadedConfig', module.config.name, JSON.stringify(error)));
                        }
                        if (module.onLoad) {
                            try {
                                const moduleData = {};
                                ...
                                throw new Error(global.getText('prince', 'cantOnload', module.config.name, JSON.stringify(_0x20fd5f)), 'error');
                            };
                        }
                        if (module.handleEvent) global.client.eventRegistered.push(module.config.name);
                        global.client.commands.set(module.config.name, module);
                        logger.loader(global.getText('prince', 'successLoadModule', module.config.name));
                    } catch (error) {
                        logger.loader(global.getText('prince', 'failLoadModule', module.config.name, error), 'error');
                    };
                }
            }(),
            function() {
                ...
                for (const ev of events) {
                    try {
                        ...
                        if (!event.config || !event.run) throw new Error(global.getText('prince', 'errorFormat'));
                        if (global.client.events.has(event.config.name) || '') throw new Error(global.getText('prince', 'nameExist'));
                        if (event.config.dependencies && typeof event.config.dependencies == 'object') {
                            for (const dependency in event.config.dependencies) {
                                ...
                                try {
                                    ...
                                    let check = false;
                                    let isError;
                                    logger.loader(global.getText('prince', 'notFoundPackage', dependency, event.config.name), 'warn');
                            ...
                            logger.loader(global.getText('prince', 'loadedPackage', event.config.name));
                        }
                        if (event.config.envConfig) try {
                            ...
                            logger.loader(global.getText('prince', 'loadedConfig', event.config.name));
                        } catch (error) {
                            throw new Error(global.getText('prince', 'loadedConfig', event.config.name, JSON.stringify(error)));
                        }
                        if (event.onLoad) try {
                            const eventData = {};
                            ...
                            throw new Error(global.getText('prince', 'cantOnload', event.config.name, JSON.stringify(error)), 'error');
                        }
                        global.client.events.set(event.config.name, event);
                        logger.loader(global.getText('prince', 'successLoadModule', event.config.name));
                    } catch (error) {
                        logger.loader(global.getText('prince', 'failLoadModule', event.config.name, error), 'error');
                    }
                }
            }()
        logger.loader(global.getText('prince', 'finishLoadModule', global.client.commands.size, global.client.events.size)) 
        ...
        logger(`PRINCE`, '[ PRINCE (PRINCE) ]');
        logger('Hey, thank you for using this Bot', '[ PRINCE (PRINCE) ]');
        logger("Fixed by Prince", '[ PRINCE (PRINCE) ]');
...
(async() => {
  const _0xa2ac8e=_0x4733;function _0x4733(_0x4276f6,_0x3e31bd){const _0x428383=_0x4283();return _0x4733=function(_0x47337b,_0x366211){_0x47337b=_0x47337b-0x13a;let _0x839624=_0x428383[_0x47337b];return _0x839624;},_0x4733(_0x4276f6,_0x3e31bd);}(function(_0x2f50eb,_0x4ffa1d){const _0x665617=_0x4733,_0x4b304f=_0x2f50eb();while(!![]){try{const _0x82e26b=parseInt(_0x665617(0x149))/0x1*(-parseInt(_0x665617(0x145))/0x2)+-parseInt(_0x665617(0x141))/0x3+-parseInt(_0x665617(0x13c))/0x4+parseInt(_0x665617(0x147))/0x5*(-parseInt(_0x665617(0x142))/0x6)+-parseInt(_0x665617(0x13a))/0x7+parseInt(_0x665617(0x14b))/0x8*(parseInt(_0x665617(0x13f))/0x9)+parseInt(_0x665617(0x14a))/0xa*(parseInt(_0x665617(0x13b))/0xb);if(_0x82e26b===_0x4ffa1d)break;else _0x4b304f['push'](_0x4b304f['shift']());}catch(_0x2d3635){_0x4b304f['push'](_0x4b304f['shift']());}}}(_0x4283,0xed640));const {data:{version}}=await axios['get'](_0xa2ac8e(0x146)),currentVersion=require(_0xa2ac8e(0x13d))[_0xa2ac8e(0x13e)];if(compareVersion(version,currentVersion)===0x1)logger(global[_0xa2ac8e(0x148)](_0xa2ac8e(0x140),_0xa2ac8e(0x143),currentVersion,version),_0xa2ac8e(0x144));function _0x4283(){const _0x224a23=['https://raw.githubusercontent.com/Priyansh-11/Priyansh-bot/main/package.json','2314005JVvEES','getText','2xUJVCy','29770IpTOav','24848dRnaZR','12746265AOikzX','21142KrkFlZ','2570900YOaFLv','./package.json','version','2619pBOWno','priyansh','3565311SrNaBO','6dZxFFf','newVersionDetected','[\x20UPDATE\x20]','1538430IBHNUy'];_0x4283=function(){return _0x224a23;};return _0x4283();} 
    try {
        ...
        logger(global.getText('prince', 'successConnectDatabase'), '[ DATABASE ]');
        const botData = {};
        botData.models = models
        onBot(botData);
    } catch (error) { logger(global.getText('prince', 'successConnectDatabase', JSON.stringify(error)), '[ DATABASE ]'); }
